...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for diet_name, diet_set in diets.items():
        if set(dish_ingredients).issubset(diet_set):
...
def tag_special_ingredients(dish:tuple):
    ...
    """
    dish_name = dish[0]
    all_ingredients = set(dish[1])
    tagged = all_ingredients.intersection(SPECIAL_INGREDIENTS)
    return (dish_name, tagged)
...
def compile_ingredients(dishes:list):
    ...
    """
    first, *therest = dishes
    return first.union(*therest)
...
def singleton_ingredients(dishes:list, intersection):
    ...
    singletons = []
    for dish in dishes:
        common = dish.intersection(intersection)
        tbd = dish.copy()
        for ing in common:
            tbd.remove(ing)
        for ing in tbd:
            singletons.append(ing)
